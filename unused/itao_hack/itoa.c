...
/* The goal is to print as quickly as possible an floating-point number.
There is no possibility of any formatting.  */
char * dtoa_fast( char *aBuf, double aD )
{
    /* Maybe we could use modf ??????????? */
    double tmpTrunc = trunc( aD );
    int tmpTruncInt = (int)tmpTrunc ;
    char * tmpRes = good_old_itoa( aBuf, tmpTruncInt );
    *tmpRes++ = '.' ;

    /* TODO: It works but not very convincing. */
    static const unsigned int biggestPowerOfTen = 1000000000UL ; /* = pow( 10, LONG_DIGITS ); */

    double tmpMantissa = ( tmpTruncInt > 0 ) ? aD - tmpTrunc : tmpTrunc -aD ;
    double mulMantissa = biggestPowerOfTen * tmpMantissa ;

    /* This is not safe at all. Maybe use lround ?? */
    unsigned int intMantissa = (unsigned int)round( mulMantissa );

    /* Drop trailing '0' chars */
    char *lastPtr = uitoa( tmpRes, intMantissa );
    for( --lastPtr ; *lastPtr == '0'; --lastPtr );
    *(++lastPtr) = '\0' ;
    return lastPtr ;
};
